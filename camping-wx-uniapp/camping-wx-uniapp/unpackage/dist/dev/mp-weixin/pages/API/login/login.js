...
const _sfc_main = {
  ...
  onLoad() {
    common_vendor.index.getProvider({
      ...
      fail: (error) => {
        common_vendor.index.__f__("log", "at pages/API/login/login.vue:107", "获取登录通道失败", error);
      }
    });
  ...
  methods: {
    ...
    tologin(provider) {
      ...
      common_vendor.index.login({
        ...
        success: async (res) => {
          common_vendor.index.__f__("log", "at pages/API/login/login.vue:137", "login success:", res);
          this.Toast({
            title: "登录成功"
          });
          this.login(provider.id);
          common_vendor.index.__f__("warn", "at pages/API/login/login.vue:157", "如需获取openid请参考uni-id: https://uniapp.dcloud.net.cn/uniCloud/uni-id");
          common_vendor.index.request({
            ...
            success(res2) {
              common_vendor.index.__f__("log", "at pages/API/login/login.vue:169", res2);
              if (res2.data.code !== 0) {
                common_vendor.index.__f__("log", "at pages/API/login/login.vue:171", "获取openid失败：", res2.data.errMsg);
                return;
              }
              common_vendor.index.setStorageSync("openid", res2.data.openid);
            },
            fail(err) {
              common_vendor.index.__f__("log", "at pages/API/login/login.vue:177", "获取openid失败：", err);
            }
          });
        },
        fail: (err) => {
          common_vendor.index.__f__("log", "at pages/API/login/login.vue:183", "login fail:", err);
          if (err.code == "30002") {
...
，错误码：${err.code}`
        ...
        common_vendor.index.__f__("error", "at pages/API/login/login.vue:254", res);
      });
    },
    async loginByApple(provider, res) {
      ...
      common_vendor.index.__f__("warn", "at pages/API/login/login.vue:288", "此处使用uni-id处理苹果登录，详情参考: https://uniapp.dcloud.net.cn/uniCloud/uni-id");
      common_vendor.index.request({
        url: "https://97fca9f2-41f6-449f-a35e-3f135d4c3875.bspapp.com/http/user-center",
        method: "POST",
        data: {
        ...
        success: (res2) => {
          common_vendor.index.__f__("log", "at pages/API/login/login.vue:297", "uniId login success", res2);
          if (res2.data.code !== 0) {
            common_vendor.index.showModal({
              showCancel: false,
